...
while (credit >= 5 && choix != "4") {
  ...
  if (choix == "1") {
    if (credit >= PLAT) {
      credit = credit - PLAT;
      alert(`Commande accepté voici votre plat il vous reste ${credit} euros`);
    } else {
  ...
  // BOISSON ALCOOL
    ...
    if (age < 18) {
      ...
      alert("Vas au travail pour gagner plus d'argent !");
    } else {
      credit = credit - BOISSON_ALCOOLISEE;
      alert(`A la tienne il vous reste ${credit} euros `);
    }
  }
  ...
  // BOISSON NON ALCOOLISEE
    ...
    if (credit >= BOISSON_NON_ALCOOLISEE) {
      ...
      alert(`Commande accepté voici votre soda il vous reste ${credit} euros `);
    } else {
      alert("Vous n'avez pas assez d'argent commandez autre chose.");
    }
  }
  ...
  alert(recupererChoixPossible(credit));
}
